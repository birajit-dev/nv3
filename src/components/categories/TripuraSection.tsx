...
async function fetchTripuraNews(): Promise<TripuraArticle[]> {
  try {
    const response = await fetch(`${API_BASE_URL}/tripura`);

    if (!response.ok) {
      throw new Error('Failed to fetch Tripura news');
    }

    const data = await response.json();
    return data.data?.tripuranews || [];
  } catch (error) {
    console.error('Failed to fetch Tripura news:', error);
    return [];
  }
}

export default function TripuraSection() {
  const [articles, setArticles] = useState<TripuraArticle[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const loadTripuraNews = async () => {
      try {
        setLoading(true);
        const tripuraNews = await fetchTripuraNews();
        setArticles(tripuraNews.slice(0, 10));
      } catch (error) {
        console.error('Failed to load Tripura news:', error);
      } finally {
        setLoading(false);
      }
    };
    ...
    return (
      <section className="mb-8">
        <div className="flex items-center justify-between mb-4 pb-2 border-b-2 border-red-600">
          <h2 className="text-2xl font-bold text-gray-900 uppercase">Tripura</h2>
    ...
    return (
      <section className="mb-8">
        ...
        <div className="bg-gray-50 p-6 text-center">
          ...
          <p className="text-gray-600">No Tripura news available at the moment.</p>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="mb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="lg:col-span-2">
          {mainArticle && (
            <Link href={`/tripura/${mainArticle.post_url}`} className="block group">
        ...
        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-3">
          {sideArticles.map((article) => (
            <Link key={article._id} href={`/tripura/${article.post_url}`} className="block group">
              <article className="bg-white border border-gray-200 hover:shadow-md transition-shadow h-full flex flex-col">
